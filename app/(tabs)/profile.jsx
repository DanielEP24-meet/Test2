...
export default function Profile() {
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      setLoading(true); // Set loading to true whenever data is being loaded
      ...
      // Extract volunteer times from user data
      if (user?.times) {
        const timesArray = Object.entries(user.times).flatMap(([orgName, dates]) => 
          Object.entries(dates).map(([date, time]) => ({
            orgName,
            date,
            time,
          }))
        );
        setVolunteerTimes(timesArray);
      }
  ...
  const VolunteerJournal = ({ volunteerTimes }) => {
    if (!volunteerTimes || volunteerTimes.length === 0) {
      return <Text style={styles.noTimesText}>No volunteer times recorded</Text>;
    }

    return (
      <View style={styles.journalContainer}>
        {volunteerTimes.map((entry, index) => (
          <View key={index} style={styles.journalEntry}>
            <Text style={styles.journalOrg}>{entry.orgName}</Text>
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.sectionTitle}>My Organizations</Text>
      <UserActivities orgs={orgs} />
      <Text style={styles.sectionTitle}>Volunteer Journal</Text>
      <VolunteerJournal volunteerTimes={volunteerTimes} />
    </ScrollView>
  );
}
